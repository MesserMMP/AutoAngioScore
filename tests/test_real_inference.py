"""Тесты с реальными DICOM файлами (или моками при их отсутствии).

Проверяет:
- Валидацию DICOM через реальные файлы или временные заглушки
- Инференс с реальными файлами (если указаны в .env.test)
- Классификацию артерий с реальными файлами
- При отсутствии реальных файлов используются моки
"""
import os
import tempfile
...
from src.web.files import is_dicom_path
...
class TestArteryClassificationRealOrMock:
    ...
    def test_classify_artery_with_real_or_mock(self):
        from src.syntax_pred.artery_cls import classify_artery
        ...
        if self.has_real_dicom_files():
            ...
            with patch('src.syntax_pred.artery_cls._load_artery_model') as mock_load, \
                 patch('src.syntax_pred.artery_cls._artery_transform') as mock_transform, \
                 patch('src.syntax_pred.artery_cls.pydicom.dcmread') as mock_dcmread, \
                 patch('src.syntax_pred.artery_cls.torch.sigmoid') as mock_sigmoid:
