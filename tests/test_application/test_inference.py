"""Тесты ML инференса: фильтрация DICOM, сортировка, объект Study, запуск пайплайна."""

import pytest
from unittest.mock import patch, MagicMock
import numpy as np
from application.infer import Study, run_inference, _filter_dicom_paths, _stable_sort_paths
...
class TestInferenceHelpers:
    ...
    def test_filter_dicom_paths_with_mock(self, sample_dicom_files):
        with patch('application.infer.pydicom.dcmread') as mock_dcmread:
...
class TestRunInference:
    ...
    def test_run_inference_no_models(self, sample_study):
        with patch('application.infer._LEFT_MODELS', []), \
             patch('application.infer._RIGHT_MODELS', []):
            result = run_inference([sample_study])
